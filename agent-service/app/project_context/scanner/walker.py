...
def _build_file_node(
    ...
    node = FileNode(
        ...
        depth=depth,
        is_dir=False,
        quick_fingerprint=quick_fingerprint(rel_path, size_bytes, mtime),
    )
    summary.total_files += 1

    decision = ignore_rules.should_skip_hash(entry, size_bytes, max_hash_bytes)
    if decision.ignored:
