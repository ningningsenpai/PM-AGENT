...
class LexiconValidator:
    ...
    def validate_manifest(
        ...
        *,
        ...
        for entry in manifest.entries:
            self._validate_required_entry_fields(manifest, entry.term_id, entry.canonical, entry.category, entry.source, issues)
            if entry.term_id in term_ids:
                issues.append(self._error("duplicate_term_id", "词条标识重复", manifest, entry.term_id))
            term_ids.add(entry.term_id)

            normalized_canonical = self.text_normalizer.normalize_for_matching(entry.canonical)
            ...
            for alias in (entry.canonical, *entry.aliases):
                ...
                if not normalized_alias:
                    issues.append(self._error("empty_alias", "别名清洗后不能为空", manifest, entry.term_id, alias))
                    continue
                if len(normalized_alias) == 1:
                    issues.append(
                        self._warning("short_alias", "单字符别名可能造成大量误召回", manifest, entry.term_id, alias)
                    )
                if normalized_alias in local_aliases:
                    issues.append(
                        self._warning("duplicate_alias", "词条内部存在重复别名", manifest, entry.term_id, alias)
                    )
                    continue
    ...
    def _validate_manifest_metadata(
        ...
        if not manifest.lexicon_id.strip():
            issues.append(self._error("empty_lexicon_id", "术语库标识不能为空", manifest))
        ...
        if manifest.scope is LexiconScope.PROJECT and not manifest.scope_id:
            issues.append(self._error("missing_scope_id", "项目术语库必须声明 scope_id", manifest))
        if manifest.status is LexiconStatus.DISABLED:
            issues.append(self._error("disabled_lexicon", "已禁用术语库不能参与构建", manifest))
        if require_published and manifest.status is not LexiconStatus.PUBLISHED:
            issues.append(self._error("unpublished_lexicon", "运行时只能加载已发布术语库", manifest))
