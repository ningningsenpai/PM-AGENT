...
class ModelCall:
    ...
    def __init__(self, body: dict, context_limit: int | None):
        ...
        if self.ledger:
            ...
            with sqlite3.connect(self.ledger, timeout=30) as conn:
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS calls (id TEXT PRIMARY KEY, reserved REAL, charged REAL, state TEXT)"
                )
                conn.execute("BEGIN IMMEDIATE")
                used = conn.execute(
                    "SELECT COALESCE(SUM(charged),0) FROM calls"
                ).fetchone()[0]
                if used + self.reserved > float(
                    os.getenv("PM_AGENT_TEST_BUDGET_CNY", "30")
                ):
                    raise ValueError(
                        "本次测试的模型费用预留超过累计预算，已停止新的付费请求"
                    )
                conn.execute(
                    "INSERT INTO calls VALUES (?,?,?,?)",
                    (self.id, self.reserved, self.reserved, "reserved"),
                )
        self.event = {
            "type": "model",
            "callId": self.id,
    ...
    def finish(self, response: dict | None = None, error: BaseException | None = None):
        usage = (response or {}).get("usage") or {}
        known = isinstance(usage.get("prompt_tokens"), int) and isinstance(
            usage.get("completion_tokens"), int
        )
        charge = (
            ((usage["prompt_tokens"] * 3 + usage["completion_tokens"] * 9) / 1_000_000)
            if known
            else self.reserved
        )
        self.event.update(
            {
                ...
                "response": safe_payload(response),
        ...
        if self.ledger:
            with sqlite3.connect(self.ledger, timeout=30) as conn:
                conn.execute(
                    ...
                    (charge, "known" if known else "estimated", self.id),
                )
        self._save()
    ...
    def _save(self):
        ...
        if root:
            ...
            (path / f"{self.id}.json").write_text(
                json.dumps(self.event, ensure_ascii=False, indent=2), encoding="utf-8"
            )
