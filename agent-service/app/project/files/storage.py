...
_SAFE_NAME_EN_PATTERN = re.compile(r"^[A-Za-z0-9._-]+$")
_SAFE_NAME_CN_PATTERN = re.compile(r"^[A-Za-z0-9._\-\u4e00-\u9fff]+$")
...
class ProjectFileStorage:
    ...
    @staticmethod
    def file_name_from_object(object_name: str) -> str:
        """从对象名中获取对外展示文件名。"""
    ...
    def _validate_file_name(file_name: str) -> str:
        ...
        if ".." in normalized or "/" in normalized or "\\" in normalized:
            raise ValueError("文件名不合法")
        if not _SAFE_NAME_CN_PATTERN.fullmatch(normalized):
            raise ValueError("文件名仅允许中文、字母、数字、点、下划线和短横线")
        return normalized
