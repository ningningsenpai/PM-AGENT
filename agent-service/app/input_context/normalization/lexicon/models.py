...
class ValidationReport(BaseModel):
    ...
    def errors(self) -> tuple[LexiconIssue, ...]:
        """返回全部错误。"""
        return tuple(
            item for item in self.issues if item.severity is IssueSeverity.ERROR
        )
    ...
    def warnings(self) -> tuple[LexiconIssue, ...]:
        """返回全部警告。"""
        return tuple(
            item for item in self.issues if item.severity is IssueSeverity.WARNING
        )
