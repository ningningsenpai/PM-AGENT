...
class LexiconMerger:
    ...
    def merge(
        ...
        resolved_entries, override_records = self._resolve_alias_conflicts(
            entries_by_term_id
        )
        ...
        version_set = LexiconVersionSet(
            ...
            project_lexicon_id=project_manifest.lexicon_id
            if project_manifest
            else None,
        ...
        for manifest in manifests:
            for entry in manifest.entries:
                ...
                if existing is None:
                    ...
                    continue
                if self._normalize(existing.canonical) != self._normalize(
                    entry.canonical
                ):
                ...
                entries_by_term_id[entry.term_id] = resolved_entry.model_copy(
                    update={
                        "aliases": self._unique_aliases(
                            (*existing.aliases, *resolved_entry.aliases)
                        ),
                        "tags": tuple(
                            dict.fromkeys((*existing.tags, *resolved_entry.tags))
                        ),
    ...
    def _resolve_alias_conflicts(
        ...
        aliases_by_term_id = {
            entry.term_id: list(entry.aliases) for entry in ordered_entries
        }
        ...
        for entry in ordered_entries:
            if (
                entry.override_term_id is not None
                and entry.override_term_id not in entries_by_term_id
            ):
                raise LexiconMergeError(
        ...
        for entry in ordered_entries:
            if (
                entry.override_term_id is not None
                and entry.term_id not in applied_override_term_ids
            ):
                raise LexiconMergeError(
        ...
        resolved_entries = [
            entry.model_copy(
                update={"aliases": tuple(aliases_by_term_id[entry.term_id])}
            )
            for entry in ordered_entries
