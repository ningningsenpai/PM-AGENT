...
from app.input_context.normalization.lexicon.json_provider import (
    DEFAULT_LEXICON_ROOT,
    JsonLexiconProvider,
)
from app.input_context.normalization.lexicon.merger import LexiconMerger
from app.input_context.normalization.lexicon.registry import LexiconRegistry
...
from app.input_context.normalization.matching import AhoMatcher, LongestMatchResolver
from app.input_context.normalization.pipeline import NormalizationPipeline
...
class NormalizationService:
    ...
    def normalize_query(
        ...
        *,
        ...
        return self._normalize(
            text, tenant_id=tenant_id, project_id=project_id, domain=domain
        )
    ...
    def validate_runtime(self, *, domain: str = "project_management") -> None:
        """预构建必需词库，确保在线请求前完成结构与发布状态校验。"""
        self.registry.get_or_build(domain=domain)
