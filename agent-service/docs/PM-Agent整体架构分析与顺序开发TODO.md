...
- MinIO、项目文件树、Qwen、内容归一化和 RAG 实验已经提前出现；
...
| 项目文件与 MinIO | 实验链路 | 已有文件 CRUD、文件树扫描和索引，但安全、权限、幂等和产品入口未定型 |
...
当前文件树 API 接收 `root_path` 和 `output_dir`，并在 Agent 服务所在服务器上直接读取和写入路径。浏览器用户选择自己电脑中的目录后，部署服务器无法直接访问该路径。
...
- 文件读取后才检查大小；
- 同步文件 IO 和 MinIO SDK 运行在异步路由中；
- 中途失败可能留下孤儿对象；
- 删除属于高风险动作，但当前可以直接调用 Python DELETE 接口。
