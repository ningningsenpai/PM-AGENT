...
Help Claude design clear, implementable, traceable business workflows and state machines so PM-Agent’s project management business and Agent automation have explicit boundaries. The first version uses lightweight state transitions only and must not introduce a full approval workflow or BPM engine.
