...
- Evaluating whether backend changes comply with the red lines and phase constraints in `AGENTS.md`.

## Goal

Help Codex design a stable, clear, evolvable Java backend architecture for PM-Agent. Keep module boundaries controlled and reduce later rework. Outputs must be directly implementable in the `backend/` project and should avoid generic advice.
...
- Follow the segments defined in `AGENTS.md` section 4.4: 0 / 1xxxx / 2xxxx / 3xxxx / 4xxxx / 5xxxx / 9xxxx.
...
1. Align with the phase in `AGENTS.md` section 5. If a capability is beyond the phase, mark it as later and do not implement it now.
