...
export function useOperation(projectId: string, conversationId?: string) {
  ...
  const valid = () =>
    ...
    localStorage.getItem('pm-agent-token') === token
  try {
    ...
    if (raw) {
      ...
      if (
        ...
        ['chat', 'learn_refine', 'development', 'risk'].includes(item.kind)
      )
        pending.value = item
    }
  } catch {
    error.value = '无法恢复本地运行记录，请从历史消息或报告查看结果'
  }
  const unresolved = computed(() =>
    Boolean(pending.value && !pending.value.terminal),
  )
  onScopeDispose(() => {
    active = false
    clearTimeout(timer)
  ...
  function save() {
    ...
    try {
      ...
      error.value = '无法保存恢复信息，请检查浏览器存储权限后重试'
      return false
    }
  }
  function accept(value: Run) {
    ...
    if (
      ...
      error.value = '运行不属于当前项目或会话'
      return
    }
    run.value = value
    if (pending.value) {
      pending.value.runId = value.runId
      pending.value.terminal = value.status !== 'running'
      if (pending.value.terminal) pending.value.payload = {}
      save()
    }
    clearTimeout(timer)
  ...
  async function recover() {
    ...
    error.value = ''
    try {
      accept(await getRun(pending.value.runId))
    } catch (e) {
      if (valid()) error.value = errorMessage(e)
    } finally {
      if (valid()) busy.value = false
    }
  }
