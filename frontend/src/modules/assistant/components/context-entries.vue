<template>
  <section class="context-panel">
    <div class="page-title-row">
      <div><h2>已记录内容</h2><p class="muted">管理当前项目的偏好、规则与记忆。明确指令自动生效，模糊内容确认后生效。</p></div>
    ...
    <n-spin :show="loading">
      <n-empty v-if="!filtered.length && !loading && !error" description="暂无符合条件的内容。可在对话中明确说明要记录的规则、偏好或目标。" />
      <article v-for="entry in filtered" :key="entry.id" class="entry-card">
        <div class="entry-meta"><n-tag size="small" :type="entryState(entry) === 'active' ? 'success' : 'default'">{{ stateLabels[entryState(entry)] }}</n-tag><span>{{ entryKinds[entry.kind] }} · 当前项目 · 版本 {{ entry.version }}</span><n-button size="small" :disabled="saving || disabled || !!pending" @click="edit(entry)">查看与纠正</n-button></div>
        <p>{{ entry.content }}</p>
    ...
    <n-modal v-model:show="editing" preset="card" title="查看与纠正内容" style="width: 720px" :mask-closable="!saving" :close-on-esc="!saving" :closable="!saving">
      <template v-if="selected">
        ...
        <details open><summary>来源</summary><blockquote v-if="selected.attributes.sourceQuote">{{ selected.attributes.sourceQuote }}</blockquote><p class="muted">{{ sourceLabel(selected) }}</p><p v-if="selected.sourceMessageId" class="muted">来源消息 {{ selected.sourceMessageId }}</p><p v-if="selected.attributes.draftId" class="muted">确认记录 {{ selected.attributes.draftId }} · 版本 {{ selected.attributes.draftVersion }}</p><details><summary>完整来源信息</summary><pre>{{ JSON.stringify(selected.attributes, null, 2) }}</pre></details></details>
...
function sourceLabel(entry: ContextEntry) {
  return ({ user_statement: '来自用户会话', user_feedback: '来自用户反馈', file_specification: '来自项目文件规范', legacy_cloud: '来自历史云端文件' } as Record<string, string>)[String(entry.attributes.sourceType)] || '项目上下文'
}
async function load() {
