  ...
  <section class="run-detail">
    ...
    <template v-else
      ...
      <p
        ...
        class="muted"
      >
        处理消息 {{ run.result.processedMessages ?? '—' }} 条，内容变更
        {{
          Array.isArray(run.result.changedEntries)
            ? run.result.changedEntries.length
            : '—'
        }}
        条。
      </p>
...
const operationLabel = computed(
  () =>
    ({ chat: '项目问答', learn: '显式学习', report: '报告生成' })[
      props.run?.operation || ''
    ] || props.run?.operation,
)
