    ...
    <aside class="workspace-sidebar">
      ...
      <nav aria-label="主导航">
        <RouterLink
          ...
          :key="item.key"
          :to="item.to"
          :class="['nav-item', { active: route.meta.section === item.key }]"
        >
          <img
            :src="route.meta.section === item.key ? activeDot : idleDot"
            alt=""
            width="12"
            height="12"
          />{{ item.label }}
    ...
    <div class="workspace-body">
      <header class="workspace-header">
        ...
        <div class="header-account">
          <n-tag v-if="useMock" type="warning" size="small">演示数据</n-tag>
          <n-dropdown :options="userOptions" @select="userAction"
            ><button class="account-button">
              <span class="user-avatar">{{
                auth.user?.username.slice(0, 1) || '用'
              }}</span
              ><span
                ><b>{{ auth.user?.username }}</b
                ><small>个人项目</small></span
              >
            </button></n-dropdown
          >
        </div>
...
const projectOptions = computed(() =>
  projects.projects.map((p) => ({ label: p.projectName, value: p.id })),
)
const navigation = computed(() => [
  ...
  ...[
    ...
    { key: 'risk', label: '风险中心' },
    { key: 'assistant', label: 'PM 助手' },
    { key: 'knowledge', label: '知识库' },
    { key: 'reports', label: '报告中心' },
  ].map((item) => ({
    ...item,
    to: projectId.value
      ? `/projects/${projectId.value}/${item.key}`
      : `/workspace/${item.key}`,
  })),
...
watch(
  () => route.params.id,
  (id) => {
    if (typeof id === 'string') projects.currentProjectId = id
  },
  { immediate: true },
)
onMounted(() => {
...
function switchProject(id: string) {
  ...
  if (route.params.id || route.path.startsWith('/workspace/')) {
    ...
    void router.push(
      section === 'projects' ? `/projects/${id}` : `/projects/${id}/${section}`,
    )
  }
}
function userAction(key: string) {
  if (key !== 'logout') {
    void router.push('/profile' + (key === 'security' ? '?tab=security' : ''))
    return
  }
  dialog.warning({
    title: '退出登录',
    ...
    onPositiveClick: async () => {
      try {
        ...
        message.warning('本地登录态已清除，服务端注销未确认')
      }
      projects.$reset()
