...
- 文件解析弹窗每 1.5 秒读取文件列表，以本轮候选文件的尝试次数变化更新进度，成功和失败均计入已处理数。文件达到 100% 后若仍在更新规范与索引，会继续显示等待状态；后端返回最终结果才解除操作锁定。
- 解析弹窗和页面入口共用忙碌状态，解析期间禁止重复提交及关闭弹窗；查询进度失败时保留已有进度并继续等待，不自动重新发起解析。
