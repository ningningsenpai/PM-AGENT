...
PM 助手点击“新建会话”直接创建并选中，名称由服务端按项目生成“项目对话-N”；标题旁的编辑图标可以修改名称并保存。历史会话、消息和运行记录各自在固定高度区域内滚动，高度随窗口调整，不随内容数量增长。

输入框使用 Enter 发送、Shift + Enter 换行，输入法组合输入和长按回车不会误发。发送后立即显示用户消息与等待提示，收到服务端历史后通过 requestKey 或运行消息 ID 替换本地消息；同样的正文仍允许在不同轮次发送。消息回复仍使用普通 HTTP，不代表已开放流式输出。
...
项目创建时需要选择本地项目文件夹。Chrome、Edge 等支持 File System Access API 的浏览器会把目录句柄按用户和项目保存在 IndexedDB，后续可直接点击“更新项目”重新扫描；本机绝对路径不会提交到后端。权限被收回时需要重新授权，换浏览器、换设备或清除站点数据后需要重新选择。不支持持久目录句柄的浏览器仅保留当前页面内的目录选择结果。
