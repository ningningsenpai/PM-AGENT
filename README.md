...
[功能概览](#功能概览) · [技术架构](#技术架构) · [资料闭环](#资料闭环) · [分层召回](#分层召回) · [多维分析](#多维分析) · [上下文更新](#上下文更新) · [工具边界](#工具调用边界) · [快速开始](#快速开始) · [阅读文档](#阅读文档) · [开源协议](#开源协议)
...
模型只负责提出结构化工具意图，不能直接访问数据库、对象存储或任意文件路径。每次调用必须依次经过请求级白名单、工具注册、参数结构、项目权限、次数与超时门禁，再由公开业务 Service 执行。

![Agent 工具调用安全边界](guide/assets/diagrams/tool-security-boundary.svg)

工具执行结果会经过结构化校验与敏感信息处理后写入运行轨迹，并以 `tool_call_id` 回填模型上下文；未知工具、非法参数、越权访问和超限调用统一收敛为可追踪的失败结果，不会被包装成业务成功。
