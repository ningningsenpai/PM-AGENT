...
> 2026-09-08 云端上下文改造后的存储、事务与新增接口以 [第 27 篇](./27-云端上下文与显式学习改造.md) 为准。下面记录初次分层重构的历史基线，其中 MySQL 正文和快照重发流程已被替换。
...
| 显式学习 | 读取消息与上下文后释放事务，再调用模型；候选应用、版本变更、学习游标和运行完成在共享事务中提交 |
| 学习失败 | 回滚尚未提交的条目、版本及游标，再单独保存失败运行并释放租约 |
| 发布快照 | 获取指定版本后释放事务，再写 MinIO；独立保存发布版本或失败原因，不回滚已提交的学习内容 |
...
- 复用会话自动编号、改名权限、请求标识、取消恢复、学习版本、快照失败重发、报告引用和幂等回归。
