...
| Java 接口 | 单文件 Multipart 上传接口和初始化解析接口 |
| Java 服务 | 文件先落库、单次上传 MinIO；队列结束后调用 Python、上传详情并补全索引 |
| MySQL | 删除批次字段，并在 `pm_project_file` 保存解析次数和索引投影字段 |
| MinIO | 保存源文件、`system/file_details/` 详情和 `system/index.json` |
...
flowchart TD
    ...
    K --> L
    L -->|否| E
    L -->|是| M["Java调用Python批量解析"]
    M --> N["按detail_ref上传详情并落库"]
    N --> O["下载并补全system/index.json"]
```
...
解析接口校验项目归属，调用 Python 批量解析，上传文件详情、保存索引投影字段并补全索引。
...
| `frontend/src/modules/project/mock.ts` | 单文件上传与初始化解析 Mock |
...
| 文件 | 作用 |
|---|---|
| `ProjectFileController.java` | 单文件上传入口 |
| `ProjectFileParseController.java` | 初始化文件解析入口 |
...
- 初始化解析接口在详情字段落库后补全 `index.json`；
- 已有文件覆盖、重命名和删除的索引行为不在本次简化范围内。

## 9. RabbitMQ 与 Agent

当前上传链路和初始化解析接口都不发布 RabbitMQ 消息、不写 Outbox。初始化解析由 Java 同步调用 Python，Python 不回调 Java，也不直接操作 MySQL 或 MinIO。
...
- 单文件上传不触发索引重建；
- Python 批量响应能够按文件校验并写入对应详情路径；
- 解析投影字段落库后 `parse_attempts` 累加；
- 补全后的索引包含详情投影字段；
...
5. 上传接口不更新索引。
6. 项目初始化索引不受影响。
7. 初始化解析接口完成 Python 调用、详情上传、字段落库和索引补全。
8. `index.json` 文件条目包含详情引用和解析投影字段。
