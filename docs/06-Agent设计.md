...
`agent_conversation` 保存项目归属和学习游标；`agent_message` 保存用户、助手消息与服务端生成的完整协议组；`agent_run` 保存请求幂等、状态、结果和轨迹。会话租约防止并发覆盖；文件解析的幂等键只识别同一次请求，`active_scope_key` 和 `lease_until` 只限制同一项目并发，二者不混用。解析使用 120 秒滑动租约，独立数据库会话每 30 秒续租；终态和取消立即释放，异常中断最长在租约过期后由恢复查询收敛。过期运行保留失败记录并允许用户使用新幂等键重试；没有 usage 的请求继续保留费用估算。
