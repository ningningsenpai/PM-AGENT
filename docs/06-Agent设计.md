...
Chat 业务按职责划分为会话、显式学习、学习上下文、运行记录和旧协议兼容子包。`chat/api.py` 仅聚合路由，`conversation/api.py`、`learning/api.py`、`context/api.py`、`runs/api.py` 分别承载对应接口；`legacy/api.py` 保留原无状态问答。报告的请求结构、模型输出约束和依赖装配归报告模块，结构化生成器装配复用 `app.llm.dependencies`。新增草稿预览、反馈和确认接口，见接口规范第 10 节。

各业务服务分别位于职责子包。`learning` 负责模型提取、定向反馈、草稿和确认；`context/store.py` 负责分类文件、版本清单、条件发布与历史；`context/migration.py` 仅导入旧内容。所有模型和 MinIO 调用之前释放数据库事务。初始化固定文件保留兼容，仅用于旧格式导入；正式内容以新版清单为准。

会话、上下文与运行分别持有自己的 Model 和 Repository。`chat/dependencies.py` 统一装配共享请求级 Session；学习用例同时使用上下文与会话仓储，运行用例同时使用运行与会话仓储，数据库负责会话游标、租约和运行记录；正式内容与草稿通过云端条件发布。仓储不自行提交，也不调用其他仓储。报告和文件解析通过公开的 `get_run_service` 获取运行服务，不构造 Chat 仓储。目录、依赖及事务详情见 [Chat 模块分层设计](./26-Chat模块分层设计.md)。
...
文件和规范沿用 system 快照的可解释词法召回：标准术语、技术标识、原始短语和中文兜底词组按来源赋予不同权重；先生成候选，再为前 `2 × limit`、最多十六个文件候选读取详情并二次排序。用户习惯只参与偏好问题，更新日志只参与变更问题；持久化会话的有效词条、习惯、规则和记忆以 MinIO 当前版本清单为准；每次读取重新校验有效状态与期限。原文补证最多两个文件、合计 12 KiB、每文件最多 100 行，并在进入模型前执行既有敏感内容阻断和脱敏，不改变相关性分数。
...
`agent_conversation` 保存项目归属和学习游标；`agent_message` 保存用户、助手消息与服务端生成的完整协议组；`agent_run` 保存请求幂等、状态、结果和轨迹。会话租约防止并发覆盖；文件解析通过 `active_scope_key` 和 `lease_until` 保证同一项目仅有一个运行中的批次，并在每个文件及上下文发布阶段前续租。过期运行保留失败记录并允许使用新幂等键重试。请求取消时保存已发生的调用、回滚未提交内容并释放租约；没有 usage 的请求继续保留费用估算。

`agent_context_entry`、`agent_context_change` 暂时保留供迁移与历史核对，迁移完成后不再读写正式正文。`agent_context_scope` 保留会话编号分配所需的锁，不再承担内容版本权威。正式版本、分类正文、来源、反馈和历史均在 MinIO。后端有限容量内存缓存保存不可变文件，Redis 仍用于认证等既有能力。

显式 learn 结构化抽取用户新消息并保存待确认草稿，任何候选都不会因模型返回 confirmed=true 而自动生效。助手回答和工具结果不作为用户学习事实。用户可编辑候选或选择一组提交自然语言反馈；模型只能整理选中主题，输出仍待确认。确认绑定草稿版本及所选内容，检查当前正式版本、引用、作用范围和冲突，再按范围条件发布。短期默认七天，直接纠正、晋升和失效不调用模型。规则文件刷新与人工学习共用项目清单，保留人工纠正。见 [改造设计](./27-云端上下文与显式学习改造.md)。
