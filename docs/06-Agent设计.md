...
项目文件详细解析是未来的受控异步 Agent 能力，不属于当前批次上传闭环。当前方案在上传请求全部成功或第三轮结束后，由 Java 同步从 MySQL 重建 `index.json`，不会发布文件业务 MQ、不会写 Outbox，也不会调用 Python。Java 最终完成服务只保留未来扩展点。后续接入时，仍应由 Java 负责权限、MySQL、MinIO 和业务状态，Python 只负责受控解析并通过 Java 接口回传结果；消息协议、幂等、重试和死信必须另行评审。当前上传权威设计见 [18-项目文件批次上传模块设计](./18-项目文件可靠上传模块设计.md)。
